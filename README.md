...
- The Compose service now uses a small wrapper entrypoint that runs `chmod og+rx /db` before delegating to the image's normal startup flow, so the Overpass CGI process can reach the runtime socket in the mounted volume without breaking the container lifecycle.
