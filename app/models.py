...
class QueryRecipeRecommendationPolicyAudit(Base):
    ...
    change_kind: Mapped[str] = mapped_column(String(32), default="manual", index=True)
    change_summary: Mapped[str] = mapped_column(String(255))
    experiment_note: Mapped[str | None] = mapped_column(String(255), nullable=True)
...
class QueryPromptClusterDecision(Base):
    ...
    __table_args__ = (
        UniqueConstraint("prompt_fingerprint", "cluster_slug", "market_country_code", name="uq_prompt_cluster_decision"),
    )
